...
public class NodeGraph {
	...
	public NodeGraph(PApplet p, Area graphArea, ControlP5 nodeControl) {
		...
		for (int i = 0; i < playerEvents.size(); i++) {
			...
			if (eType == EventTypes.KILL) {
				...
				if (kill.getKiller().equalsIgnoreCase(playerName)) {
					...
					toolTipText = "Event Type: Kill" + newLineCharacter
							+ "Weapon: " + kill.getWeapon() + newLineCharacter
							+ "Victim: " + kill.getVictim();
				} else if (kill.getVictim().equalsIgnoreCase(playerName)) {
					playerScore -= kill.getValue();
					toolTipText = "Event Type: Death" + newLineCharacter
							+ "Weapon: " + kill.getWeapon() + newLineCharacter
				...
				toolTipText = "Event Type: Defence" + newLineCharacter
						+ "Defended Object: " + defend.getDefendedObject();
			} else if (eType == EventTypes.CAPTURE) {
				CaptureEvent capture = (CaptureEvent) currentEvent;
				playerScore += capture.getValue();
				toolTipText = "Event Type: Defence" + newLineCharacter
						+ "Capture Description: " + capture.getCapturedObject();
			}

			// Get the difference between the initial time and the current event
			// to calculate X position.
			int timeDifference = (int) getDateDiff(firstEventTime,
